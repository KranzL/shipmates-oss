...
var (
	...
	discordChannelCache   = make(map[string]discordChannelEntry)
	discordChannelCacheMu sync.RWMutex
	discordChannelLRUList = list.New()
	discordChannelLRUMu   sync.Mutex
...
func InitPipelineNotifier(client *discordgo.Session) {
...
func evictExpiredEntries() {
	...
	discordChannelCacheMu.Lock()
	for key, value := range discordChannelCache {
		if value.expiresAt.Before(now) {
			delete(discordChannelCache, key)
			discordChannelLRUMu.Lock()
			if value.element != nil {
				discordChannelLRUList.Remove(value.element)
			}
			discordChannelLRUMu.Unlock()
		}
	}
	discordChannelCacheMu.Unlock()
...
func notifyScheduleResult(ctx context.Context, db *goshared.DB, userID string, result ScheduleResult) {
	...
	if err != nil {
		fmt.Printf("[issue-pipeline] Failed to fetch workspaces for user %s: %v\n", userID, err)
		return
	}
	...
	for _, workspace := range workspaces {
		if workspace.Platform == goshared.PlatformDiscord {
			...
			if err := sendDiscordNotification(ctx, workspace.PlatformID, message); err != nil {
				fmt.Printf("[issue-pipeline] Failed to send Discord notification: %v\n", err)
			}
			...
			if err := sendSlackNotification(ctx, db, workspace.PlatformID, message); err != nil {
				fmt.Printf("[issue-pipeline] Failed to send Slack notification: %v\n", err)
			}
		}
	}
}

func resolveDiscordChannel(ctx context.Context, guildID string) (*discordChannelEntry, error) {
	discordChannelCacheMu.RLock()
	cached, exists := discordChannelCache[guildID]
	discordChannelCacheMu.RUnlock()

	if exists && cached.expiresAt.After(time.Now()) {
		discordChannelLRUMu.Lock()
		discordChannelLRUList.MoveToFront(cached.element)
		discordChannelLRUMu.Unlock()
		return &cached, nil
	}
	...
	if client.State == nil || client.State.User == nil {
		return nil, fmt.Errorf("discord client state not initialized")
	}
	...
	if guild.SystemChannelID != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("get guild channels: %w", err)
		}
	...
	if targetChannelID == "" {
		return nil, fmt.Errorf("no writable text channel found in guild %s", guildID)
	}

	discordChannelCacheMu.Lock()
	discordChannelLRUMu.Lock()
	if len(discordChannelCache) >= maxCacheSize {
		back := discordChannelLRUList.Back()
		if back != nil {
			oldKey := back.Value.(string)
			delete(discordChannelCache, oldKey)
			discordChannelLRUList.Remove(back)
		}
	}
	elem := discordChannelLRUList.PushFront(guildID)
	entry := discordChannelEntry{
		channelID: targetChannelID,
		expiresAt: time.Now().Add(time.Duration(discordCacheTTLMs) * time.Millisecond),
		element:   elem,
	}
	discordChannelCache[guildID] = entry
	discordChannelLRUMu.Unlock()
	discordChannelCacheMu.Unlock()
...
func sendDiscordNotification(ctx context.Context, guildID, message string) error {
	...
	resolved, err := resolveDiscordChannel(ctx, guildID)
	if err != nil {
		return err
	}
	...
	_, err = client.ChannelMessageSend(resolved.channelID, truncated)
	if err != nil {
